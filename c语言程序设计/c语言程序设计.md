...
![image-20240625105340077](assets/image-20240625105340077.png)

![image-20240625105401601](assets/image-20240625105401601.png)
...
小端序（Little-Endian）和大端序（Big-Endian）是计算机系统中关于多字节数据在内存中存储顺序的两种标准。以下是关于小端序和大端序的详细解释：

### 1. 基本概念

- **字节序（Byte Order）**：指多字节数据在内存中的存放顺序。
- **大端序（Big-Endian）**：高位字节存储在内存的低地址处，低位字节存储在内存的高地址处。
- **小端序（Little-Endian）**：低位字节存储在内存的低地址处，高位字节存储在内存的高地址处。

### 2. 存储方式

- **大端序**：例如，对于一个16位整数`0x1234`，在大端序中，它将被存储为`0x12`（高位字节）在内存的低地址处，`0x34`（低位字节）在内存的高地址处。
- **小端序**：同样地，对于一个16位整数`0x1234`，在小端序中，它将被存储为`0x34`（低位字节）在内存的低地址处，`0x12`（高位字节）在内存的高地址处。

### 3. 示例（以4个字节的整数为例）

假设有一个32位整数`0x12345678`，按照大端序和小端序的存储方式如下：

- **大端序**：内存中从低到高的地址依次存储`0x12`、`0x34`、`0x56`、`0x78`。
- **小端序**：内存中从低到高的地址依次存储`0x78`、`0x56`、`0x34`、`0x12`。

### 4. 如何判断

在编程中，可以使用特定的方法来判断当前机器或处理器使用的是大端序还是小端序。一种常用的方法是通过存储一个整数值（如`0x00000001`）并检查其字节表示来确定。

### 5. 应用场景

- **底层编程**：在底层编程或涉及硬件交互的编程中，了解和处理大小端问题非常重要。
- **跨平台数据交换**：当在不同平台或系统之间进行数据交换时，需要确保数据的字节序一致，以避免数据解析错误。

### 6. 常见的处理器和平台

- **x86系列CPU**：通常使用小端序。
- **PowerPC**：通常使用大端序。
- **其他处理器**：一些处理器可以通过配置或跳线来设置使用大端序还是小端序。

### 总结

小端序和大端序是计算机系统中关于多字节数据在内存中存储顺序的两种标准。了解和处理大小端问题对于底层编程和跨平台数据交换至关重要。不同的处理器和平台可能使用不同的字节序，因此在编程时需要注意这一点。